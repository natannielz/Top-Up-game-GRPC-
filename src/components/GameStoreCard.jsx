...
import { Star, ShoppingCart, Gamepad2, Gift } from 'lucide-react';
...
const GameStoreCard = ({ game }) => {
  ...
  return (
    <Link to={`/topup/${game.id}`} className="block group relative">
      <div className="relative h-[25rem] rounded-2xl overflow-hidden transition-all duration-500 transform group-hover:-translate-y-2 group-hover:shadow-[0_20px_50px_rgba(188,19,254,0.15)] bg-cyber-dark border border-white/5 group-hover:border-cyber-purple/50">
        ...
        <div className="absolute top-4 left-4 flex gap-2">
          <span className="backdrop-blur-md bg-black/40 border border-white/10 text-white text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1">
        ...
        {discount > 0 && (
          <div className="absolute top-4 right-4 animate-bounce-slow">
            <span className="backdrop-blur-md bg-gradient-to-r from-red-600 to-pink-600 text-white text-xs font-bold px-3 py-1 rounded-full shadow-[0_0_15px_rgba(255,0,0,0.5)]">
              -{discount}%
            </span>
          </div>
        )}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 z-10">
          <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
            <div className="flex justify-between items-start mb-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100 absolute -top-8 w-full">
              <div className="flex items-center gap-1 bg-black/60 backdrop-blur rounded px-2 py-0.5 border border-white/10">
