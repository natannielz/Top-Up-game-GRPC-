          ...
          <div className="flex items-center gap-4">
            ...
            {user ? (
              <div className="relative group">
                <Link to="/dashboard" className="relative p-0.5 rounded-full bg-gradient-to-r from-cyber-cyan to-cyber-purple hover:shadow-glow-cyan transition-all block">
                  <img
                    src={user.avatar}
                    alt={user.username}
                    className="w-10 h-10 rounded-full object-cover border-2 border-cyber-black"
                  />
                </Link>
                {/* Mini Dropdown */}
            ...
            {searchQuery.length > 1 && (
              <div className="mt-2 space-y-2">
                {games.filter(g => g.title.toLowerCase().includes(searchQuery.toLowerCase())).slice(0, 3).map(g => (
                  <div key={g.id} onClick={() => { navigate(`/topup/${g.id}`); setIsMobileMenuOpen(false); }} className="flex items-center gap-3 p-2 bg-white/5 rounded-lg active:bg-white/10">
