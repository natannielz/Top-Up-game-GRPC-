...
const NewsPage = () => {
      ...
      try {
        ...
        if (newsData && Array.isArray(newsData)) {
          const mappedNews = newsData.map((item, index) => {
            ...
            let fallbackImage = 'https://images.unsplash.com/photo-1614726365723-49cfa080998a?q=80&w=2670&auto=format&fit=crop';

            // If we have specific media for this game ID, use it
            if (mediaData && mediaData[item.gameId] && mediaData[item.gameId].length > 0) {
              const assets = mediaData[item.gameId];
              // Use a deterministic rotation based on index so it doesn't flicker on re-render
              fallbackImage = assets[index % assets.length];
            } else if (mediaData && mediaData['1091500']) {
              // Fallback to Cyberpunk (1091500) assets if specific game assets missing
              const genericAssets = mediaData['1091500'];
              fallbackImage = genericAssets[index % genericAssets.length];
            }

            // Automatic Signal Classification
            let signalCategory = 'LORE & NARRATIVE';
            const text = (item.news_title + item.content).toLowerCase();
            if (text.match(/patch|update|fix|v\d+\.|hotfix|changelog|notes/)) signalCategory = 'PATCH NOTES';
            else if (text.match(/esport|major|tournament|cup|prize|qualifier|final|champ/)) signalCategory = 'ESPORTS';

            return {
              id: `ext-${index}`,
              title: item.news_title || 'United Network Update',
              summary: (item.content || '').replace(/<[^>]*>?/gm, '').substring(0, 150) + '...',
  ...
  // Derived Lists
  const allGames = useMemo(() => Array.from(new Set(newsItems.map(i => i.gameName).filter(Boolean))), [newsItems]);
