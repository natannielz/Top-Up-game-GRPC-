...
const TransactionPage = () => {
  ...
  }, [initGameKey, setStatus]);

  // Denominations Hardcoded for Demo (In real app, fetch from props/API)
  const denominations = [
    { id: 1, amount: '5', price: 1500 },
    { id: 2, amount: '10', price: 2850 },
    { id: 3, amount: '50', price: 14250 },
    { id: 4, amount: '100', price: 28500 },
    { id: 5, amount: '250', price: 71250 },
    { id: 6, amount: '500', price: 142500 }
  ];
                  ...
                  {denominations.map(denom => (
                    <button
                      ...
                      className={`relative p-4 rounded-xl border text-left transition-all duration-300 hover:-translate-y-1
                                        ${selection.item?.id === denom.id
                          ? 'bg-gradient-to-br from-cyber-cyan/20 to-blue-600/20 border-cyber-cyan shadow-[0_0_15px_rgba(0,255,255,0.2)]'
                          : 'bg-white/5 border-white/5 hover:border-white/20'
                        }`}
                    >
                      <div className="text-xl font-bold font-orbitron text-white mb-1">{denom.amount}</div>
                      <div className="text-xs text-gray-500 uppercase tracking-wider mb-2">{game.currency}</div>
                      <div className={`text-sm font-bold ${selection.item?.id === denom.id ? 'text-cyber-cyan' : 'text-gray-300'}`}>
                        Rp {denom.price.toLocaleString()}
                      </div>
                      {selection.item?.id === denom.id && <Check size={16} className="absolute top-3 right-3 text-cyber-cyan" />}
                    </button>
                  ))}
                </div>
              ) : (
                ...
                <div className="space-y-4 relative z-10">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-white font-bold">
                      {selection.item ? (isTopUp ? `${selection.item.amount} ${game.currency}` : 'Standard Key') : '-'}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
