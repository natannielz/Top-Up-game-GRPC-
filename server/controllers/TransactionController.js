  ...
  try {
    ...
    if (mockPayment.status === 'SUCCESS' && productType === 'TOPUP') {
      ...
      if (ACTIVE_PROVIDER === 'DIGIFLAZZ') { // --- DIGIFLAZZ FLOW ---
        ...
        // Map Response VIP
        if (providerResult.result === true) {
          gameDeliveryResponse = {
            status: 'PROCESSING', // VIP returns immediate success means "Queued/Entered"
            message: providerResult.data?.message || 'Order Masuk ke VIP Reseller',
            sn: providerResult.data?.trxid || '', // Using TRXID as SN for now since SN comes later
          ...
          gameDeliveryResponse = {
            status: 'FAILED',
            message: providerResult.message || 'Gagal Order ke VIP Reseller',
            sn: null,
            provider: 'VIP_RESELLER'
          };
        }
